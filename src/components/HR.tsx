...
export function HR() {
  ...
  const [tab, setTab] = useState<'payroll' | 'advances' | 'children'>('payroll');
  ...
  const [drafts, setDrafts] = useState<Record<string, { allowances: string; feeDeduction: string; otherDeductions: string; notes: string }>>({});
  ...
  const setDraft = (teacherId: string, field: string, value: string) =>
                  ...
                  <div className="px-5 py-4 grid grid-cols-2 md:grid-cols-6 gap-3 items-end">
                    ...
                    <div className="col-span-2 md:col-span-6">
                      <input value={d.notes} onChange={e => setDraft(t.id, 'notes', e.target.value)}
                        ...
                        className="w-full px-2.5 py-1.5 border border-gray-200 rounded-lg text-xs focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                    </div>
                  </div>
                </div>
              );
